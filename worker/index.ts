...
const json = (c:any,data:any,status=200) =>
  c.json(data,status);
...
const requirePerm=(p:string)=>(c:any)=>{
...
app.post(
  ...
  async c=>{
    ...
    if(
      body.secret!==c.env.BOOTSTRAP_SECRET
    )
      return json(
        c,
        {error:'Forbidden'},
...
app.post(
  '/api/admin/events/:id/attendance/confirm',
  async c=>{
    const d=admin(c);
    if(d)return d;
  ...
  async c=>{
    const reqs=await c.env.DB
      .prepare(`
        ...
        ORDER BY rank,visible_order,id
      `)
      .all<any>();
...
/*
 * Admin member detail / position management
 */
...
app.put(
  '/api/admin/members/:id/positions',
  async c=>{
    const d=admin(c);
    if(d)return d;
    ...
    const ids=Array.isArray(x.positionIds)
      ? x.positionIds
          .map(Number)
    ...
    const valid=await c.env.DB
      .prepare(
        'SELECT id,category FROM positions'
      )
      .all<any>();
    ...
    if(
      ...
      return json(
        ...
        {
          error:
            'One or more positions are not valid for this member type'
        },
        400
      );
    ...
    await c.env.DB
      .prepare(
        'DELETE FROM person_positions WHERE person_id=?'
      )
      .bind(id)
      .run();
  ...
  async c=>{
    ...
    if(!rel)
      return json(
        ...
        {
          error:
            'Only a connected parent/guardian can sign'
        },
        403
      );
